...
CREATE TABLE IF NOT EXISTS fiduciaries (
    ...
    email VARCHAR(255) UNIQUE, -- Changed to UNIQUE for clarity, though not strictly in schema
    phone VARCHAR(50),
    address TEXT,
    ...
    dpo_user_id UUID, -- FK to users(id) - will be added later if users table exists
    ...
    last_updated_at TIMESTAMPZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS consent_policies (
    ...
    fiduciary_id UUID NOT NULL REFERENCES fiduciaries(id),
    effective_date TIMESTAMPZ NOT NULL,
...
CREATE TABLE IF NOT EXISTS consent_records (
    ...
    timestamp TIMESTAMPZ NOT NULL, -- When consent was given/updated
    jurisdiction VARCHAR(5) NOT NULL,
    ...
    -- This constraint is tricky with NULLs. A partial unique index is better if user_id can be NULL for anonymous.
    -- For simplicity here, assuming user_id is always set.
...
CREATE TABLE IF NOT EXISTS grievances (
    ...
    submission_timestamp TIMESTAMPZ NOT NULL DEFAULT NOW(),
    ...
    resolution_timestamp TIMESTAMPZ,
    last_updated_at TIMESTAMPZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS api_keys (
    ...
    fiduciary_id UUID NOT NULL REFERENCES fiduciaries(id),
    processor_id UUID REFERENCES processors(id), -- Nullable if key is for Fiduciary app directly
...
CREATE TABLE IF NOT EXISTS purge_requests (
    ...
    initiated_at TIMESTAMPZ NOT NULL DEFAULT NOW(),
...
INSERT INTO roles (id, name, description, permissions) VALUES
    (uuid_generate_v4(), 'Admin', 'Full system administration access', '["*:*"]'::jsonb),
    (uuid_generate_v4(), 'DPO', 'Data Protection Officer role with compliance oversight', '["policy:read", "policy:publish", "consent:read", "grievance:manage", "audit:read", "dpb_report:submit", "fiduciary:read", "processor:read", "retention:read", "purge:read", "purge:execute"]'::jsonb),
    (uuid_generate_v4(), 'Auditor', 'Read-only access for auditing purposes', '["policy:read", "consent:read", "audit:read", "grievance:read", "fiduciary:read", "processor:read", "retention:read", "purge:read"]'::jsonb),
    (uuid_generate_v4(), 'Operator', 'Limited access for operational tasks', '["grievance:manage_limited", "purge:execute_limited"]'::jsonb);

-- Add a default admin user (IMPORTANT: CHANGE PASSWORD AFTER INITIAL SETUP)
-- You should replace 'admin_password_hash_here' with a strong hash generated by your application
-- For a real system, initial admin creation should be part of a secure setup process, not hardcoded.
INSERT INTO users (id, username, email, password_hash, mfa_enabled, status) VALUES
    (uuid_generate_v4(), 'superadmin', 'admin@tsicoop.org', 'your_initial_admin_password_hash_here', FALSE, 'ACTIVE');

-- Assign the 'Admin' role to the default admin user
INSERT INTO user_roles (user_id, role_id) VALUES
    ((SELECT id FROM users WHERE username = 'superadmin'), (SELECT id FROM roles WHERE name = 'Admin'));