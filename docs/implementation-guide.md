...
**Audience:** DPOs, Data Stewards, and the engineering team that configures policies with them - not infrastructure setup.

Four related guides cover the technical side of running the system:

- [Production Deployment Guide](production-deployment-guide.md) - for DevOps/system administrators hardening a Docker or Binary install before go-live.
- [System Integration Guide](system-integration-guide.md) - for developers at Data Fiduciaries/Processors calling the client API to capture consent and validate processing.
- [Webhook Integration Guide](webhook-integration-guide.md) - for developers wiring the CMS's notification/purge/OTP webhooks into downstream systems.
- [Client Polling Integration Guide](polling-integration-guide.md) - the reliable, always-available alternative to webhooks for the same notification/purge integrations.
